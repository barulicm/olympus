...
pub fn render_scorecard(
    ...
    tt.add_template(
        "scorecard",
        include_str!("templates/scorecard.html.template"),
    )?;
    tt.render("scorecard", &build_context(game_description.clone()))
}

pub fn render_score_calculator(
    game_description: &GameDescription,
) -> Result<String, tinytemplate::error::Error> {
    let mut tt = TinyTemplate::new();
    tt.add_template(
        "calculator",
        include_str!("templates/score_calculator.js.template"),
    )?;
...
fn format_question_in_mission_id(question_index: usize) -> String {
    format!(
        "{}",
        char::from_u32(('a' as u32) + question_index as u32).unwrap_or('?')
    )
}
...
fn format_question_id(mission_index: usize, question_index: usize) -> String {
    format!(
        "M{}{}",
        mission_index,
        format_question_in_mission_id(question_index)
    )
}

fn build_context(game_description: GameDescription) -> Context {
    Context {
        missions: game_description
            .missions
            .iter()
            .enumerate()
            .map(|(mission_index, mission)| MissionContext {
                ...
                questions: mission
                    ...
                    .iter()
                    .enumerate()
                    .map(|(question_index, question)| QuestionContext {
                        ...
                        show_plus_on_max: if let QuestionDescription::Number {
                            show_plus_on_max,
                            ..
                        } = question
                        {
